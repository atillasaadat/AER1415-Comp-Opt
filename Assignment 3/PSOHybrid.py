...
Descripton: Question 5 - Hybrid PSO Algorithm - SLSQP
...
class ParticleHybrid:
	...
	def tunedVelocityFunc(self,velParams,posGlobBest,getVel=False):
		vel_cognitive = velParams[1]*(self.posBest-self.pos.copy())
		vel_social = velParams[2]*(posGlobBest-self.pos.copy())
		#calcualte new particle velocity
		newVel = velParams[0]*self.vel + vel_cognitive + vel_social
		if getVel:
			return newVel
		return self.costFunc(add(self.pos,newVel),penaltyParam=self.penaltyParam,params=self.params)
	...
	def update_position(self,posGlobBest):
		...
		x0 = [self.params['w'],self.params['c1'],self.params['c2']]
		bnds = [(0,1),(0,4),(0,4)]
		optVelParams = minimize(self.tunedVelocityFunc,x0,args=posGlobBest,bounds=bnds,method='SLSQP', tol=1e-4, options={'maxiter': 100}).x
		self.vel = self.tunedVelocityFunc(optVelParams,posGlobBest,getVel=True)
...
class PSOHybrid:
	...
	def optimize(self,verbose=False):
		...
		for repeatIter in range(self.maxRepeat):
			self.swarm = [ParticleHybrid(self.costFunc,self.x0,self.bounds,self.params) for i in range(self.numParticles)]
